...
Shader "Water FX/Ocean Shader Plus" {
...
Properties {
	...
	_CubeMap ("Reflection Cubemap", Cube) = "" { TexGen CubeReflect }
	
	_Reflectivity ("Reflectivity", Range (1,0)) = .5
...
CGPROGRAM
	...
	void surf (Input IN, inout SurfaceOutput o) {
		...
		half4 reflcol = texCUBE (_CubeMap, worldRefl);
		
		half4 col;
		
		col.rgb = _MainColor.rgb;
		col.rgb = lerp(reflcol.rgb, col.rgb, _Reflectivity);
		col.a = _MainAlpha;
